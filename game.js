...
class Game {
    ...
    remainingAttempts() {
        return Math.max(0, 3 - this.getAttemptsCount());
    }
    ...
    incrementAttempt() {
        ...
        localStorage.setItem('flavour_attempts_count', String(Math.min(3, current + 1)));
        this.updateAttemptsUI();
    }
    ...
    updateAttemptsUI() {
        ...
        const startBtn = document.getElementById('startBtn');
        if (el) {
            el.textContent = `المحاولات المتبقية اليوم: ${this.remainingAttempts()} من 3`;
        }
        if (startBtn) {
    ...
    showNoAttemptsMessage() {
        alert('لقد استخدمت كل محاولاتك اليوم (3). الرجاء المحاولة غدًا.');
    }
